...
# Unified opening suite (UCI move-lines) generated by tools/build_opening_suite.py — the SINGLE SOURCE
# OF TRUTH shared by the 5% self-play seed AND the arena gate. Loaded once at import (re-imported in
# spawned workers, same as the fallback list). If the file is absent/empty/unreadable, fall back to the
# 20-line list above → behaviour is byte-identical to the old hardcoded book (so no box without the
# suite breaks). Same UCI move-line format, so the existing self-play book machinery is unchanged.
def _load_opening_suite():
    path = os.path.join(os.path.dirname(__file__), "openings_suite.txt")
    try:
        with open(path) as fh:
            lines = [ln.strip() for ln in fh if ln.strip() and not ln.startswith("#")]
        return lines if lines else _FALLBACK_OPENING_BOOK
    except Exception:
        return _FALLBACK_OPENING_BOOK

OPENING_BOOK = _load_opening_suite()
# Arena plays the SAME suite, color-paired (see run_arena_server_worker) so the gate is broad + fair.
ARENA_OPENINGS = OPENING_BOOK
...
def run_arena_server_worker(worker_id, n_games, cand_in_q, cand_out_q, champ_in_q, champ_out_q,
    ...
    try:
        for gi in range(n_games):
            ...
            # Opening: play a balanced suite opening as a COLOR-BALANCED PAIR — opening k (= game_id//2)
            # is played once with the candidate as White (even game_id) and once as Black (odd), so the
            # opening's imbalance cancels (AZ/Lc0 eval methodology). Both trees + history track every
            # book ply; deterministic argmax play resumes after. Replaces the old uniform-random move
            # (which handed an engine a junk opening and never color-paired → unfair, noisy gate).
            if ARENA_OPENINGS:
                opening = ARENA_OPENINGS[(game_id // 2) % len(ARENA_OPENINGS)]
                for u in opening.split():
                    if game.is_over:
                        break
...
def _champ_sf_score(iteration):
    ...
    (win_rate, elo, elo_hi) — elo_hi is BayesElo's CI UPPER bound, the bar a candidate must CLEAR to
    promote. Cache: model/champ_sf.json."""
    try:
    ...
    if os.path.exists(CHAMP_SF_CACHE):
        try:
            ...
            pass
    wr, elo, r = _sf_eval(BEST_MODEL, iteration, "champ")   # same game count as the candidate
    elo_hi = r.get("elo_upper") if r else None
    if wr is not None:
        try:
...
def run_evaluation_phase(iteration, logger, p_loss, v_loss, train_metrics=None):
    ...
    if sf_gate:
        ...
        cleanup_memory()
        champ_wr, champ_elo, champ_hi = _champ_sf_score(iteration)   # re-measures on best_model OR anchor change
        cleanup_memory()
        ...
        if champ_wr is None or cand_wr is None:
            ...
            # Promote only when the candidate is CLEARLY better: its Elo must clear the champion's
            # BayesElo CI UPPER bound, not merely tie the point estimate. A >= point-estimate gate
            # promoted coin-flips (it103-107: 4 promotions in 5 iters, all marginal, champion flat at
            # ~2470) — at parity, half of equal-strength candidates clear the bar by noise alone. This
            # bar is self-calibrating: more games → tighter CI → smaller edge required.
            bar = champ_hi if champ_hi is not None else champ_elo   # CI missing → fall back to point est
            promote = cand_elo > bar
            verb = "⭐ PROMOTED" if promote else "rejected"
            print(f" [SF-gate] {verb} — cand Elo {cand_elo:.0f} {'>' if promote else '<='} champ CI-upper "
                  f"{bar:.0f} (champ Elo {champ_elo:.0f})  (WR {cand_wr*100:.1f}% vs {champ_wr*100:.1f}%)")
        else:
    ...
    if sf_gate:
        gate_markers = {"sf_gate_champ_wr": None if champ_wr is None else round(champ_wr, 4),
                        ...
                        "sf_gate_champ_hi": champ_hi}   # the promotion bar (champion's CI upper)
